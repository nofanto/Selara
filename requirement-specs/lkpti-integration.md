...
>
> **Decided at the same time — "has gone live", not "is live right now".** The amendment raised a case §3 had never confronted: a Deliverable that went live and whose live segment has since *ended* (in the demo template, "Azure AD B2C" — in production until mid-2026, out of support from mid-2027, currently in a gap). Two readings were on the table:
>
> - **Has gone live** (chosen) — any live segment with `startDate <= today`. An application the bank actually ran stays on the filing until removed deliberately.
