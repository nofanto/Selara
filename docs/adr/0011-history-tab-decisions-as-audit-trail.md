...
## Field coverage note (issue #42)

The history comparison is used as an audit trail for workspace state, so silently missing a changed field undermines its purpose. `requirement-specs/version-history-field-coverage.md` records the current field audit and the exhaustive policy used by `computeDiff`: every compared entity field is classified, and a test requires every field marked `diffed` to produce a change message. Decision records remain outside `computeDiff` under the rule above and appear in the separate decisions-in-span section.
