...
- Choose **It’s this existing entry** when the inventory already lists the application or infrastructure. Search the same-kind entries and select one yourself; suggestions are never selected automatically. If it has no earlier live history, the form says that a prior live phase will be added. For each filed attribute that differs from what the entry would file, choose **Update** or **Keep**. Updating changes the inventory entry and therefore the LKPTI too; keeping files the entry’s current value. A PPJTI developer update requires the provider’s actual name. Confirm creates the implementation and clears the finding in one undoable change.
- An older RPTI import may have a one-year prior live phase that leaves an application out of later inventories. A non-blocking warning names the application and the missing years. A recorded retirement ends this check: only missing year-ends before the earliest later phase that is neither live nor pre-launch (for example Sunset or Retired) are flagged. Review the warning and choose **Extend** to confirm extending the prior phase through the planning horizon or to the day before that retirement, whichever comes first. A later Planned or Funded phase does not end the check, and a live phase merely ending still warrants review. Nothing changes automatically when the workspace opens, and a prior phase you have edited is not flagged. **Undo** restores the original phase and warning; the warning does not block RPTI export.
