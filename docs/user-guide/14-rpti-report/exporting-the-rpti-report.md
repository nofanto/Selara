...
If generation cannot resolve an initiative’s target, the report names the problem and repair and withholds export. A complete source repair may span three places: create or correct the item on **Data Manager → Deliverables**, choose it in the **Deliverable** column on **Data Manager → Initiatives**, and add a qualifying lifecycle segment linking that Initiative and Deliverable on the **Visualiser timeline**. Existing unsupported Asset-target rows require the same migration. If the Deliverable and target are already in place, the gate names only the remaining timeline segment; for an ambiguous target, select the intended Deliverable or split the work into one initiative per target.
