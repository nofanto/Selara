...
- **Resize vertically**: drag the **bottom edge** of a segment bar downward to make it span multiple row heights. This is useful to show one deliverable replacing several that previously ran in parallel. The handle appears on hover at the bottom of the bar. Drag upward to collapse back to a single row.

All changes save automatically when you release the mouse.

## Stacking and rows

When two segments for the same deliverable overlap in time, they are automatically placed in separate rows within the swimlane. The swimlane height expands to accommodate them.
