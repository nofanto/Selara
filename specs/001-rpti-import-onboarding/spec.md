...
- **FR-019a**: An **infrastructure** upgrade row (category `51`-`54`, `99`) with **no** match MUST have its entry created rather than left unresolved. FR-019 withholds creation because the two returns are known to disagree on naming, making a non-match a judgement call; that reasoning applies to applications, which both returns list. LKPTI is *Daftar Aplikasi* and never contains infrastructure at all, so an infrastructure non-match is a certainty rather than a disagreement, and there is nothing for a person to judge. Left unresolved it was a dead end — a data-health error no import could ever clear, because no LKPTI could supply the target. The created entry MUST carry a **live** segment rather than a planned one (FR-021), which is what makes regeneration classify it as an upgrade, matching what was filed. Ambiguity is still deferred: this applies only when **nothing** matched, so several same-named infrastructure entries remain unresolved, and a later import that finds this entry attaches to it rather than creating a second.
- **FR-020**: An RPTI row filed as **new** MUST create a new entry without consulting the inventory.
- **FR-021**: A row that creates its own entry MUST produce **exactly one** lifecycle segment, spanning the quarter it filed, and that segment's status MUST state whether the thing exists yet: an `upgrade` is **live**, because the bank already runs it; a `new` build is **planned**. No live period is invented for a `new` build — the return files an intention to build, never an outcome, and asserting a go-live nobody filed would put it on the timeline and, for an application, into the LKPTI. A `new` application therefore reports as having no live segment until someone marks it shipped, which is correct and is surfaced as a completeness warning.
- **FR-022**: A `Deliverable` MUST only ever be created together with its own `Asset`, never attached to an existing one, so an imported deliverable never hangs off an asset belonging to something else.
- **FR-023**: A row attaching to an **existing** entry MUST add the planned segment, and MUST add a preceding live segment **only when that entry has no live segment starting before the report year**. The entry already carries its own history; duplicating it drew a second lifecycle bar wholly inside the first, and a "Planned" period spanning years the inventory records as live — one application reading as three on the timeline. The conditional preserves the reason the segment exists at all: a target built by hand may have no live history, and without it the filed `upgrade` regenerates as `new`. No trailing live segment is produced, consistent with FR-021.
