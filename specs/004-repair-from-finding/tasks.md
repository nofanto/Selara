...
- [x] T044 **SC-004.** Add a pinned test in `src/lib/sampleReturns.test.ts` for LKPTI counts as at 31 December 2026-2032, in two states: after import, and after the Legacy Teller B repair. Copy the measured numbers into Q22 in `requirement-specs/report-rows-as-projections.md`, with an explanation of each difference from T002's baseline and from Q21's 13, 16, 16, 16, 16, 16, 3. Include the confirmed consequence of 2026 going from 13 to 14. The test is written after the code it measures, so it MUST be shown to have teeth (Constitution III, added after `/speckit-analyze` C2): revert T040, then T019, and measure the pinned counts each time. **Coordinator correction, 2026-09-26:** T040 is expected not to move these counts because the sample's only synthetic prior phase is infrastructure; T035/T036 cover that rule. Disabling T019 must fail the post-repair counts. Log to `/tmp/selara-004-t044-falsify.log`.
  - Done 2026-09-26: new SC-004 test measures and pins import-only **13/16/16/16/16/16/3** and after B **14/17/17/17/17/17/4**, copied to Q22 with every difference explained. Measurement `/tmp/selara-004-t044-measure.log` (11:15:47 WIB); falsification `/tmp/selara-004-t044-falsify.log` (11:16:06-11:16:07 WIB): T040 reversal **1 passed**, T019 writes disabled **1 failed** on the post-repair counts, restored **1 passed**. Existing expectations unchanged.
- [x] T045 [P] Update `docs/user-guide/14-rpti-report/recording-an-rpti-row.md`:
  ...
  - Done 2026-09-26: recording guide now describes B and A, confirmation, prior history, one-step Undo and Cancel, with the manual fallback retained. The deleted-implementation paragraph is byte-identical. Added the older prior-phase warning and **Extend** to Data Health. Searched `docs/user-guide` and `README.md` for unresolved/unmatched repairs and target selectors; the exporting and Data Manager guides already describe Repair and segment-owned targets, with no stale target dropdown found.
- [x] T046 [P] **FR-019 check.** `git diff 9e38187 -- src/types.ts` shows no change to a stored entity, and `src/lib/diffFieldPolicy.test.ts` is green unmodified. If either fails, stop and declare the field in `src/lib/diffFieldPolicy.ts` test-first.
  - Done 2026-09-26: `git diff 9e38187 -- src/types.ts` and the same diff for `src/lib/diffFieldPolicy.test.ts` are empty. Unmodified field-policy suite **104/104**, exit 0, `/tmp/selara-004-final-field-policy.log` (11:16:22 WIB); no stored-field policy change required.
- [x] T047 Mark spec 004's status as implemented. Add a line to Q22 pointing at spec 004 as the feature that shipped it.
  - Done 2026-09-26: spec status is **Implemented (2026-09-26)**. Q22 links the implemented spec. The retirement/prior-phase-warning question raised in the US3 review was initially recorded open, then decided by the product owner on 2026-09-26 as option B; T049 implements that follow-up.
- [x] T048 Run quickstart levels 1-4 with real exit codes, and hand the product owner the manual check, including SC-006's timings. Unit, Playwright full suite, eslint 0 errors, and a tsc error count of exactly 1 (`excel.ts:295`). Log each to `/tmp/selara-004-final-*.log`, and record the totals here.
  - Done 2026-09-26, including T049: Level 1 **293/293** (`/tmp/selara-004-final-level1.log`, 11:22:29 WIB); Level 2 **25/25** (`…-samples.log`, 11:22:29-11:22:30); Level 3 **8 passed, 1 flaky**, exit 0 (`…-e2e.log`, 11:22:33-11:22:55). Full unit **640/640** in 28 files, exit 0 (`…-unit-full.log`, 11:22:30-11:22:31); full Playwright **695 passed, 3 flaky, 4 skipped**, exit 0 (`…-e2e-full.log`, 11:22:55-11:26:00). ESLint **0 errors, 8 warnings**, exit 0 (`…-eslint.log`); TypeScript **exactly 1** baseline `excel.ts:295` TS2322, exit 2 (`…-tsc.log`). Every Playwright run had a clear port 3000 preflight; no existing server was reused or killed. The product-owner handoff, including unperformed SC-006 B <60s / A <120s timings, is in `/tmp/selara-004-polish-report.md`.
  - Focused confirmation: the three flaky full-suite files plus the repair workflow pass **42/42 with `--retries=0`**, exit 0 (`/tmp/selara-004-final-e2e-rerun.log`, 11:26:29-11:26:48 WIB). The final full run and this rerun both pass the repair workflow on the first attempt. Final source audit `/tmp/selara-004-final-review.log` confirms no existing expectation edits or stored-field changes.

- [x] T049 **Product-owner follow-up, 2026-09-26 — retirement ends the inventory-gap check (Q22 option B).** Record the decision and rejected A/C/D alternatives before code; amend FR-018a. Test-first in `src/lib/unresolvedRowRepair.test.ts`: no gap after mid-2028 retirement when 2027 is covered; retirement in 2030 leaves exactly uncovered 2027-2029; Extend ends before retirement; later Planned/Funded phases do not count as retirement; no post-live segment preserves the old behaviour. Use the earliest post-live start after the original prior year's end on the same Deliverable, ignoring live and pre-launch phases. Implement in `priorPhaseGaps` and `extendImportPriorPhase`, keep Data Health wording accurate, and update the user guide. Red `/tmp/selara-004-t049-red.log`, green `/tmp/selara-004-t049-green.log`, falsification `/tmp/selara-004-t049-falsify.log`; include in the final full suites. Existing expectations must remain unchanged.
  - Done 2026-09-26: Q22 records option B and rejected A/C/D before implementation; FR-018a, Data Health wording, guide and originating story agree. Added 11 cases: red **7 failed, 4 passed**, green **123/123** repair/health tests, falsification **7 failed**, restored **11/11**. Logs `/tmp/selara-004-t049-{red,green,falsify}.log`; the shared earliest-post-live calculation bounds both missing years and Extend, including strict year-end, midyear and leap-day boundaries. No existing expectation changed; T048 final suites include T049.
...
- **Polish (T044-T049)**: T044 needs T019 and T040. T049 extends T041/T042 after the product-owner decision and is included in T048's final suites. The documentation tasks need the stories they document.
