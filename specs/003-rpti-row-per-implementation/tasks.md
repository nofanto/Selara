...
- [ ] T001 Branch from the merged `main` (currently `5100509`) as `003-rpti-row-per-implementation`, and cherry-pick or rebase the unpushed `7a75726` from `052-multi-implementation-detection` so the #52 detection this feature removes is present to remove.
- [ ] T002 [P] Re-measure the three symptoms in `research.md` R1 on the new base and record the numbers in this file, so the "before" figures quoted in tests are this branch's, not `7a75726`'s.
...
- [ ] T006 Add `capexAmount`, `opexAmount` and `rptiRemarks` to `DeliverableSegment` in `src/types.ts`, each optional, each with a comment naming the filed column it supplies and pointing at Q17.
...
- [ ] T007 [P] [US1] Failing test in `src/lib/rpti.test.ts`: two live segments on one application starting Q2 and Q4 of the filed year produce **two** rows with quarters Q2 and Q4 (contract 4). Today: one row, Q4.
- [ ] T008 [P] [US1] Failing test in `src/lib/rpti.test.ts`: a segment starting in 2027 and running to 2031 produces a row in **2027 only** (contract 3). Today: a row in every year until the segment ends.
- [ ] T008a [P] [US1] Failing test in `src/lib/rpti.test.ts`: an implementation falling outside the filed year produces no row **and no reconciliation finding** (contract 7). Absence from a year is correct, not unreproducibility — the companion rule from Q11.
- [ ] T009 [P] [US1] Failing test in `src/lib/rpti.test.ts`: a pre-launch phase followed by a live phase in the same year is **one** implementation, so one row, typed `new` (contract 5).
- [ ] T010 [P] [US1] Failing test in `src/lib/rpti.test.ts`: an initiative with implementations on two different applications produces **two** rows, one per application (contract 6).
- [ ] T010a [P] [US1] Failing test in `src/lib/rpti.test.ts`: two implementations of the **same** application produce rows carrying identical application-level values — category, developer, related party, DC/DR locations, platform (FR-007). Newly at risk: one application can now produce several rows, so a cascade bug would surface as two rows disagreeing about the same application.
- [ ] T011 [P] [US1] **Regression guard** in `src/lib/rpti.test.ts`: a workspace where every initiative has exactly one implementation produces byte-identical rows to today — same values, same order (SC-003, contract 8). This is the test most likely to catch an over-broad change, and almost every real workspace is this shape.
- [ ] T012 [P] [US1] Failing e2e in `e2e/report-year.spec.ts`: generate 2027 from a workspace with two implementations and see two rows in the Reports view, then both in the exported workbook.
