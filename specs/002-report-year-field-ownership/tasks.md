...
- [ ] T018 [US1] Surface unreproducible rows before export in `src/components/ReportsView.tsx`, naming what to repair (FR-024). `dataHealth.ts` already raises `rpti-target` for the unresolved import — reuse that finding rather than inventing a second one.
...
- [ ] T022 [US2] Give `generateLkptiDetails` a required `asAtDate` parameter in `src/lib/lkpti.ts`, replacing `startDate <= today` with a span test `startDate <= asAt && endDate >= asAt` (research.md R2). **This also fixes a measured defect**: a decommissioned application currently still appears.
...
- [ ] T026 [US2] Assert in `e2e/report-year.spec.ts` that the Reports-menu path does not mutate stored rows (contract 3), and that `src/components/DataManager.tsx`'s report tabs and generate buttons are untouched (FR-021).
...
- [ ] T028 [US3] Repoint the data-health checks that report these values as missing at the entity now holding them, updating both message and `location` (FR-018). `lkpti-incomplete` must no longer send the preparer to a tab where the value cannot be fixed.
...
- [ ] T036 [P] Mark Q1–Q3 as implemented in `requirement-specs/report-rows-as-projections.md`.
- [ ] T037 [P] Update `docs/user-guide/` — the Reports menu now asks for a year, and the eight attributes are recorded on the application rather than on the LKPTI row.
- [ ] T038 **Release note** in the PR description and `docs/adr/` entry from T033: a decommissioned application will drop out of the generated LKPTI. Correct, previously intended, but a behaviour change that must be stated rather than shipped quietly (research.md R2).
