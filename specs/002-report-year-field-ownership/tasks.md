...
- [x] T021 [P] [US2] Add `e2e/report-year.spec.ts`, failing: the Reports menu asks for a year before producing either return, and both the rendered result and the parsed downloaded workbook state the selected year.
...
- [x] T026c [US2] Lift any `RptiDetail.capexAmount`/`opexAmount` that differs from its initiative's figure onto the initiative, then remove those legacy properties and persist the cleaned rows as a durable one-time completion signal (FR-031). Imported overrides equal the initiative already; a hand-edited one does not, and dropping it before the lift is the silent loss this feature exists to prevent.
...
- [x] T026h [US2] Add a data-health **error** for any `RptiDetail` with `targetType: 'asset'`, naming the complete repair — record the item as a Deliverable under its Asset on the Deliverables tab, point the initiative at it on the Initiatives tab, and add a qualifying segment for that pair on the Visualiser timeline (FR-032, FR-033) — in `src/lib/dataHealth.ts`. Turns T026g green.
...
- [x] T026a [US2] Make both report tabs read-only in `src/components/DataManager.tsx` and remove their two Generate buttons (FR-021). Drops the asset entries from the Initiatives tab's Deliverable dropdown (`:208-210`) as a consequence. *(Unblocked by Q7 and Q8; depends on T026d, T026h and T018's export gate — the tab must not become read-only while an unreproducible row can still reach a filing unannounced.)* The tabs stay present and populated; nothing in them accepts an edit. *(Revised 2026-09-18 — the earlier task asserted the opposite, that the tabs were untouched.)*
- [x] T026 [US2] Assert in `e2e/report-year.spec.ts` that the Reports-menu path does not mutate stored rows (contract 3), and that neither report tab accepts an edit or offers a generate action (FR-021).
- [x] T026i [US2] **Issue #40 defect — option 3 guardrail (Q11).** Replace `generateRptiDetails` with `projectRptiReturn(input, reportYear)` over a `ProjectRptiInput` that has no `existingDetails` key, and delete `mergeWithExisting` (its only production caller was the defective one). Assert the compile-time guard with a `@ts-expect-error` test. In `src/lib/rpti.ts`, `src/components/ReportsView.tsx`, and the migrated unit tests.
- [x] T026j [US2] **Issue #40 defect — option 1 reconciliation (Q11/Q12).** Add `reconcileRptiReturn(input)` returning typed findings (reason codes, source-side repairs, row-as-evidence), wired into the Reports pre-export gate beside the source diagnostics. Reconcile by one-to-one canonical identity rather than immutable stored ids, and prove by E2E that following the complete named Asset-target repair — including adding the qualifying Initiative/Deliverable segment on the Visualiser timeline — clears the finding and enables export. Verified LKPTI carries no equivalent membership hazard; new LKPTI imports retain target-name identity evidence while already-unlabelled orphans direct re-import.

## Phase 7: Adversarial review corrections (2026-09-19)

- [x] T033 [F1] Pass the selected year into both exporters, add a metadata worksheet and year-bearing filename, and prove workbook content by downloading and parsing both files in Playwright.
- [x] T034 [F2] Replace immutable-id-only RPTI reconciliation with one-to-one canonical identity correspondence; retain target-name evidence on new LKPTI imports and give already-orphaned rows an honest re-import repair.
- [x] T035 [F3] Make the legacy cost lift one-time by removing and immediately persisting the lifted row properties; prove a later Initiative edit survives reload.
