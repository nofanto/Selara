...
- [x] T026j [US2] **Issue #40 defect — option 1 reconciliation (Q11).** Add `reconcileRptiReturn(input)` returning typed findings (reason codes, source-side repairs, row-as-evidence), wired into the Reports pre-export gate beside the source diagnostics. Table-driven unit test over the four states the merge conflated; E2E proving a 2027 line stays out of a 2026 return while an unreproducible stored row is named and blocks export. Verified LKPTI carries no equivalent membership hazard (its `existingDetails` refreshes rows that already qualify — pinned by the existing drop-out test in `src/lib/lkpti.test.ts`).
