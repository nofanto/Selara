...
- **FR-025**: Repairing what the data-health review flags MUST be sufficient to make the round trip faithful. Nothing MUST require the preparer to re-key a value the return already supplied. A repair message MUST name every outstanding source-side step and where to perform it: create or correct the Deliverable on **Data Manager → Deliverables** when needed, select it in the Initiative's **Deliverable** column on **Data Manager → Initiatives**, and add a qualifying Initiative/Deliverable lifecycle segment on the **Visualiser timeline**. Once the entity and target steps are complete, the finding SHOULD name only the remaining segment step rather than repeat completed work.
...
- **FR-027**: Because that comparison is manual, the system MUST make a filed row that cannot be reproduced at all loud rather than leaving its absence to the reader — FR-024 is what carries this. Field-level drift on a row that still has one unambiguous canonical counterpart is deliberately not reported. *(Revised 2026-09-19: stored rows are evidence of what was filed, not assertions about what the evolving portfolio should generate now; identity reconciliation is actionable, while warnings for detectable-but-reproducible content drift would make the gate noisy without changing the correct regeneration.)*

**Not losing what is already there**

- **FR-019**: A workspace created before this change MUST NOT silently lose the eight attributes. The values MUST be carried onto the applications before any regeneration can replace the rows holding them whenever old-shaped data enters live state through ordinary IndexedDB load, shared-workspace load, generic workbook import, or version restore, and the lifted form MUST be persisted immediately. Cross-tab sync need not repeat the lift because it reads the writing tab's already-lifted, persisted state. Exported workbooks outside Selara cannot be reached in place; they MUST be lifted when re-imported.
...
- **FR-033**: An existing row targeting a bare Asset MUST raise a data-health **error** before export, naming the complete repair — record the item on **Data Manager → Deliverables**, select it in the Initiative's **Deliverable** column on **Data Manager → Initiatives**, and add a qualifying segment for that Initiative/Deliverable pair on the **Visualiser timeline**. Error, not warning: no generated return can reproduce such a row, so it otherwise leaves the filing in silence.
...
- **SC-005**: A workspace created before this change retains its attributes through regeneration after ordinary load, shared-workspace load, generic workbook import, or version restore. Silent loss occurs in no live-state entry path; an exported workbook outside Selara is lifted when re-imported.
...
- **Existing workspaces are few and known.** Broad in-place migration tooling remains out of scope (Q4), but the shipped idempotent lift covers every reachable boundary where old-shaped data can enter live state.
...
- Eagerly rewriting every existing workspace, saved snapshot, or exported file in place. The boundary lift on ordinary load, shared-workspace load, workbook import and version restore is in scope; files outside Selara are necessarily handled only when re-imported.
